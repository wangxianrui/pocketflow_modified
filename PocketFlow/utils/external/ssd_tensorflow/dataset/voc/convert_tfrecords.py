...
'''How to organize your dataset folder:
  VOCROOT/
       ...
       |    |->...
'''
tf.app.flags.DEFINE_string('dataset_directory', '/home/hzh/DATA/VOCdevkit',
                           'All datas directory')
...
# tf.app.flags.DEFINE_string('output_directory', '/home/wxrui/DATA/VOCdevkit/tfrecords',
...
output_directory = os.path.join(FLAGS.dataset_directory, 'tfrecords')
...
def _process_dataset(name, directory, all_splits, num_shards):
    ...
    if name == 'val':
        for split in all_splits:
            jpeg_file_path = os.path.join(directory, split, 'ImageSets/Main/test.txt')
            images = tf.gfile.GFile(jpeg_file_path, 'r').read().splitlines()
            jpegs = [name + '.jpg' for name in images]
            all_records.extend(list(zip([split] * len(jpegs), jpegs)))
    elif name == 'train':
        for split in all_splits:
            jpeg_file_path = os.path.join(directory, split, 'ImageSets/Main/trainval.txt')
            images = tf.gfile.GFile(jpeg_file_path, 'r').read().splitlines()
            jpegs = [name + '.jpg' for name in images]
            all_records.extend(list(zip([split] * len(jpegs), jpegs)))
...
def main(unused_argv):
    if os.path.exists(output_directory):
        print('file path {} already exist'.format(output_directory))
        return
