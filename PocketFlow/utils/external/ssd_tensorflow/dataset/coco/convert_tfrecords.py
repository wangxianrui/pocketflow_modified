...
'''How to organize your dataset folder:
    COCOROOT/
        ...
        |->images
            ...
            |->val2014
'''
tf.app.flags.DEFINE_string('dataset_directory', '/DATA/coco',
                           'All datas directory')
tf.app.flags.DEFINE_string(
    'train_splits', 'train2014, val2014',
    'Comma-separated list of the training data sub-directory')
tf.app.flags.DEFINE_string(
    'validation_splits', 'val2014',
...
def _convert_to_example(filename, image_id, image_buffer, bboxes, labels,
                        height, width):
...
def _find_image_bounding_boxes(directory, cur_record, coco):
    ...
    Returns:
      ...
      truncated: List of ints indicate the truncation of that bounding box.
    """
    coco_img = coco.loadImgs(ids=cur_record)[0]
    shape = [coco_img['width'], coco_img['height'], -1]
    anna_id = coco.getAnnIds(imgIds=cur_record)
    anna_file = coco.loadAnns(ids=anna_id)
    bboxes = []
    labels = []
    for anna in anna_file:
        bbox = anna['bbox']
        x_min = bbox[0] / shape[0]
        y_min = bbox[1] / shape[1]
        x_max = (bbox[0] + bbox[2]) / shape[0]
        y_max = (bbox[1] + bbox[3]) / shape[1]
        bboxes.append([y_min, x_min, y_max, x_max])
        label = dataset_common.labelmap[str(anna['category_id'])]
        labels.append(label)
    return bboxes, labels


def _process_image_files_batch(coder, thread_index, ranges, name, directory,
                               all_records, num_shards, coco):
    ...
    for s in range(num_shards_per_batch):
        ...
        for i in files_in_shard:
            ...
            filename = coco.loadImgs(ids=cur_record)[0]['file_name']
            if 'train' in filename:
                filename = os.path.join(directory, 'images', 'train2014',
                                        filename)
            else:
                filename = os.path.join(directory, 'images', 'val2014',
                                        filename)

            bboxes, labels = _find_image_bounding_boxes(
                directory, cur_record, coco)
            try:
                image_buffer, height, width = _process_image(filename, coder)
            except tf.errors.InvalidArgumentError:
                print('This image is corrupted, remove or fix it:', filename)
                continue

            if bboxes != [] and labels != []:
                example = _convert_to_example(filename, cur_record, image_buffer,
                                              bboxes, labels, height, width)
...
def _process_image_files(name, directory, all_records, num_shards, coco):
    ...
    print(
        'Launching %d threads for spacings: %s' % (FLAGS.num_threads, ranges))
    sys.stdout.flush()
    ...
    for thread_index in range(len(ranges)):
        args = (coder, thread_index, ranges, name, directory, all_records,
                num_shards, coco)
...
def _process_dataset(name, directory, all_splits, num_shards):
    ...
    if name == 'val':
        coco = COCO(
            os.path.join(FLAGS.dataset_directory, 'annotations',
                         'minival.json'))
    elif name == 'train':
        coco = COCO(
            os.path.join(FLAGS.dataset_directory, 'annotations',
                         'trainval.json'))
    all_records = coco.getImgIds()
    shuffled_index = list(range(len(all_records)))
    random.seed(RANDOM_SEED)
    random.shuffle(shuffled_index)
    all_records = [all_records[i] for i in shuffled_index]
    _process_image_files(name, directory, all_records, num_shards, coco)
...
def create_jsonfile():
    import json
    print('load jsonfile.......')
    path = os.path.join(FLAGS.dataset_directory, 'annotations')
    train2014 = json.load(
        open(os.path.join(path, 'instances_train2014.json'), 'r'))
    val2014 = json.load(
        open(os.path.join(path, 'instances_val2014.json'), 'r'))

    train_list = open('trainval.txt', 'r').read().splitlines()
    train_list = [int(os.path.splitext(ll)[0][-7:]) for ll in train_list]
    val_list = open('minival.txt', 'r').read().splitlines()
    val_list = [int(os.path.splitext(ll)[0][-7:]) for ll in val_list]

    print('split jsonfile....')
    val = dict()
    train = dict()
    ...
    print('write json file back....')
    json.dump(train, open(os.path.join(path, 'trainval.json'), 'w'))
    json.dump(val, open(os.path.join(path, 'minival.json'), 'w'))
...
def main(unused_argv):
    ...
    # Run it!
    _process_dataset('val', FLAGS.dataset_directory,
                     parse_comma_list(FLAGS.validation_splits),
                     FLAGS.validation_shards)
    _process_dataset('train', FLAGS.dataset_directory,
                     parse_comma_list(FLAGS.train_splits), FLAGS.train_shards)
